    ...
    public partial class Form1 : Form
    {
        ...
        private void pictureBox1_Click(object sender, EventArgs e)
        {
            ...
            if (!point_button.Enabled)
            {
                ...
                if (prim_edge._p1.X > 0 && prim_edge._p2.X > 0)
                    ...
                    ShowInfo();
                if (prim_polygon._done)
                    AddInfo($"Inside convex: {IsInConvexPolygon()}, \r\n Inside non convex: {IsInNonConvexPolygon()}");
                RedrawScene();
            }
        ...
        {
            var prev_pol = prim_polygon._start;
            var cur_pol = prim_polygon._start._next;
            for (int i = 1; i < prim_polygon._size; i++)
            {
                var l = new Line(prev_pol.ToPoint(), cur_pol.ToPoint());
                if (point_position_to_edge(prim_point, l, false) != 1)
                    return false;
                prev_pol = cur_pol;
                cur_pol = cur_pol._next;
            }
            return true;
        }
        ...
        private bool IsInNonConvexPolygon()
        {
            ...
            var cur = prim_polygon._start._next;
            while (cur != prim_polygon._start)
            {
            ...
            while (true)
            {
                ...
                double y = Sin(angle) * a;
                double x = Cos(angle) * a;
                Point p1 = new Point(prim_point.X + (int)x, prim_point.Y + (int)y);
                Line l1 = new Line(prim_point, p1);

                PolygonPoint prev = prim_polygon._start;
                cur = prim_polygon._start._next;
                bool need_cont = false;
                int count = 0;
                while (true)
                {
                    Line l2 = new Line(prev.ToPoint(), cur.ToPoint());
                    Point p = GetIntersect(l1, l2);
                    if (p.X == -1)
                    {
                        prev = cur;
                        cur = cur._next;
                        if (prev == prim_polygon._start)
                            break;
                        continue;
                    }
        ...
        private int point_position_to_edge(Point p, Line l, bool Default_Dir = true)
        {
            Point up = l._p2;
            Point down = l._p1;
            if (Default_Dir)
            ...
            double xp = p.X - down.X;
            double xup = up.X - down.X;
            double yp = down.Y - p.Y; 
            double yup = down.Y - up.Y;
            if (yp * xup - xp * yup > 0)
                return -1;
            if (yp * xup - xp * yup < 0)
                return 1;
